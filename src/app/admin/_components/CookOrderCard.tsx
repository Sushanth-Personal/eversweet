...
export function CookOrderCard({
  ...
  const [sentToDelivery, setSentToDelivery] = useState(false);
  ...
  return (
    ...
    >
      ...
      >
        ...
        />
        <button
          disabled={sendingToDelivery || sentToDelivery}
          onClick={async () => {
            setSendingToDelivery(true);
            await onSendToDelivery(order);
            setSendingToDelivery(false);
            setSentToDelivery(true);
          }}
          style={{
            flex: 1,
            minWidth: "20%",
            padding: "12px 10px",
            background: sentToDelivery ? G.greenGlass : "rgba(240,176,64,0.14)",
            border: "none",
            color: sentToDelivery ? G.green : G.gold,
            fontSize: 13,
            fontWeight: 600,
            cursor:
              sendingToDelivery || sentToDelivery ? "not-allowed" : "pointer",
            opacity: sendingToDelivery ? 0.6 : 1,
            fontFamily: "system-ui, sans-serif",
            display: "flex",
            alignItems: "center",
            justifyContent: "center",
